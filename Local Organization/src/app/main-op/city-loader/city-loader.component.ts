...
import { attributesMapping, fakenameAttributesMapping, ICountry } from './city-loader.constants';
...
@Component({
    ...
    standalone: true,
    imports: [NgIf, NgForOf, FormsModule, InputTextModule, ButtonModule, AccordionModule],
...
export class CityLoaderComponent implements OnInit {
    ...
    constructor(private cityLoaderService: CityLoaderService) {}

    ngOnInit() {}

    loadCountriesAndCities() {
        this.cityLoaderService.getLocalSheets('worldcities_3.csv').subscribe(sheets => {
            this.sheets = sheets;
    ...
    loadFakeUsernames() {
        this.cityLoaderService.getLocalSheets('fakenames.xlsx').subscribe(sheets => {
            this.sheets = sheets;
...
(\'` +
    ...
    private loadCities() {
        ...
        const countries = Array.from(new Set(rowsCountries));
        ...
        console.log(
            'Romania: ',
            this.countries.find(c => this.isSame(c.country, 'Romania')),
        );
